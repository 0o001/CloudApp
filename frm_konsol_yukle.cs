...
namespace CloudApp
{
    public partial class frm_konsol_yukle : Form
    {
        public string link;
        public string kod;
        public frm_konsol_yukle()
        {
            InitializeComponent();
        }
        ...
        private void frm_program_yukle_Load(object sender, EventArgs e)
        {
            wb.Encoding = Encoding.UTF8; 
            wb.DownloadProgressChanged += new DownloadProgressChangedEventHandler(wb_DownloadProgressChanged);
            wb.DownloadStringCompleted += new DownloadStringCompletedEventHandler(wb_DownloadStringCompleted);
            wb.DownloadStringAsync(new Uri("http://mustafauzun0.16mb.com/KONSOL/"+link));
        }
        void Calistir(string kod) //Kod derleme
        {
            ...
            parameters.GenerateInMemory = false; //Hafızada yer tutmaması için
           ...
           //System.Diagnostics.Process.Start("Kod.exe");
            try
            {
